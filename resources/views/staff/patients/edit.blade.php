<x-staff-layout>
    <div class="container mx-auto px-4 py-6">
        <div class="max-w-2xl mx-auto">
            <!-- Header -->
            <div class="mb-6">
                <h1 class="text-2xl font-bold text-gray-900 mb-2">Edit Data Pasien</h1>
                <p class="text-gray-600">Perbarui informasi data pasien</p>
            </div>
            ...
            <div class="bg-white rounded-lg shadow-md p-6">
                <form action="{{ route('staff.patients.update', $patient) }}" method="POST" class="space-y-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="bg-gray-50 p-4 rounded-md">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                            ...
                            <div>
                                <span class="text-gray-600">Status Registrasi:</span>
                                @if($patient->user_id)
                            ...
                            <div>
                                ...
                                <span class="font-medium">{{ $patient->created_at->format('d/m/Y H:i') }}</span>
                            </div>
                            <div>
                                <span class="text-gray-600">Terakhir Diperbarui:</span>
                                <span class="font-medium">{{ $patient->updated_at->format('d/m/Y H:i') }}</span>
                            </div>
                        </div>
                    </div>
                ...
                <div class="flex items-center">
                    ...
                    <div class="flex-1">
                        ...
                        <p class="text-sm text-red-700 mt-1">
                            Menghapus data pasien akan menghapus semua informasi terkait termasuk riwayat pemeriksaan. Tindakan ini tidak dapat dibatalkan.
                        </p>
                    </div>
                    <form action="{{ route('staff.patients.destroy', $patient) }}" method="POST" class="ml-4"
                          onsubmit="return confirm('Apakah Anda yakin ingin menghapus data pasien {{ $patient->name }}? Tindakan ini tidak dapat dibatalkan!')">
                        @csrf
                        @method('DELETE')
                        <button type="submit" 
